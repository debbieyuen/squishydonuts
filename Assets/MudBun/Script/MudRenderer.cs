...
namespace MudBun
{
  ...
  public class MudRenderer : MudRendererBase
  {
    ...
    */
    ...
    /// <returns>Result containing the SDF value, plus material if <c>computeMaterial</c> is true. The normal field of the result would be left empty.</returns>
    public Sdf.Result EvaluateSdf(Vector3 p, float maxSurfaceDistance, bool computeMaterials)
    {
      UpdateComputeData();
      s_aSingleSampleSync[0] = p;
      Sdf.EvaluateSdf(Sdf.AsyncMode.None, this, s_aSingleSampleSync, s_aSingleResultSync, m_aSdfBrush, m_numSdfBrushes, m_aSdfBrushMaterial, m_aabbTree.NodePods, m_aabbTree.Root, maxSurfaceDistance, computeMaterials, SurfaceShift);
    ...
    /// <p/>
    /// You must call <c>Complete()</c> on the returned job handle no later than the next <c>MudRenderer.LateUpdate</c>, which could alter input data used by the evaluation job.
    /// </summary>
    ...
    /// <returns>A job handle that can be waited on for completion. The normal field of the result would be left empty.</returns>
    ...
    {
      UpdateComputeData();
      var hJob = Sdf.EvaluateSdf(Sdf.AsyncMode.Async, this, samples, results, m_aSdfBrush, m_numSdfBrushes, m_aSdfBrushMaterial, m_aabbTree.NodePods, m_aabbTree.Root, maxSurfaceDistance, computeMaterials, SurfaceShift);
      m_jobQueue.Add(hJob);
      return hJob;
    }

    /*
    public Sdf.EvalJobHandle EvaluateSdfAsyncInputCopied(NativeArray<Vector3> samples, NativeArray<Sdf.Result> results, float maxSurfaceDistance, bool computeMaterials)
    {
      UpdateComputeData();
      var hJob = Sdf.EvaluateSdf(Sdf.AsyncMode.AsyncInputCopied, this, samples, results, m_aSdfBrush, m_numSdfBrushes, m_aSdfBrushMaterial, m_aabbTree.NodePods, m_aabbTree.Root, maxSurfaceDistance, computeMaterials, SurfaceShift);
      m_jobQueue.Add(hJob);
      return hJob;
    }
    */
    ...
    /// <param name="p">The sample position.</param>
    /// <param name="maxSurfaceDistance">The range of evaluation from the sample position. Only brushes within this range are considered for evaluation. Use a conservative estimate of distance from the SDF's zero isosurface.</param>
    /// <returns>Result containing the SDf normal. The SDF value field of the result would be left empty.</returns>
    public Sdf.Result EvaluateNormal(Vector3 p, float maxSurfaceDistance)
    {
      UpdateComputeData();
      s_aSingleSampleSync[0] = p;
      Sdf.EvaluateNormal(Sdf.AsyncMode.None, this, s_aSingleSampleSync, s_aSingleResultSync, m_aSdfBrush, m_numSdfBrushes, m_aSdfBrushMaterial, m_aabbTree.NodePods, m_aabbTree.Root, maxSurfaceDistance, SurfaceShift, 1e-3f);
      return s_aSingleResultSync[0];
    }
    ...
    /// <param name="computeMaterials">Whether to compute the materials at sample points as well.</param>
    /// <returns>A job handle that can be waited on for completion. The SDF value field of the result would be left empty.</returns>
    ...
    {
      UpdateComputeData();
      var hJob = Sdf.EvaluateNormal(Sdf.AsyncMode.Async, this, samples, results, m_aSdfBrush, m_numSdfBrushes, m_aSdfBrushMaterial, m_aabbTree.NodePods, m_aabbTree.Root, maxSurfaceDistance, SurfaceShift, 1e-3f);
      m_jobQueue.Add(hJob);
      return hJob;
    }

    /*
    public Sdf.EvalJobHandle EvaluateNormalAsyncInputCopied(NativeArray<Vector3> samples, NativeArray<Sdf.Result> results, float maxSurfaceDistance)
    {
      UpdateComputeData();
      var hJob = Sdf.EvaluateNormal(Sdf.AsyncMode.AsyncInputCopied, this, samples, results, m_aSdfBrush, m_numSdfBrushes, m_aSdfBrushMaterial, m_aabbTree.NodePods, m_aabbTree.Root, maxSurfaceDistance, SurfaceShift, 1e-3f);
      m_jobQueue.Add(hJob);
      return hJob;
    }
    */
    ...
    public Sdf.Result EvaluateSdfAndNormal(Vector3 p, float maxSurfaceDistance, bool computeMaterials)
    {
      UpdateComputeData();
      s_aSingleSampleSync[0] = p;
      Sdf.EvaluateSdfAndNormal(Sdf.AsyncMode.None, this, s_aSingleSampleSync, s_aSingleResultSync, m_aSdfBrush, m_numSdfBrushes, m_aSdfBrushMaterial, m_aabbTree.NodePods, m_aabbTree.Root, maxSurfaceDistance, computeMaterials, SurfaceShift, 1e-3f);
      return s_aSingleResultSync[0];
    }
    ...
    {
      UpdateComputeData();
      var hJob = Sdf.EvaluateSdfAndNormal(Sdf.AsyncMode.Async, this, samples, results, m_aSdfBrush, m_numSdfBrushes, m_aSdfBrushMaterial, m_aabbTree.NodePods, m_aabbTree.Root, maxSurfaceDistance, computeMaterials, SurfaceShift, 1e-3f);
      m_jobQueue.Add(hJob);
      return hJob;
    }

    /*
    public Sdf.EvalJobHandle EvaluateSdfAndNormalAsyncInputCopied(NativeArray<Vector3> samples, NativeArray<Sdf.Result> results, float maxSurfaceDistance, bool computeMaterials)
    {
      UpdateComputeData();
      var hJob = Sdf.EvaluateSdfAndNormal(Sdf.AsyncMode.AsyncInputCopied, this, samples, results, m_aSdfBrush, m_numSdfBrushes, m_aSdfBrushMaterial, m_aabbTree.NodePods, m_aabbTree.Root, maxSurfaceDistance, computeMaterials, SurfaceShift, 1e-3f);
      m_jobQueue.Add(hJob);
      return hJob;
    }
    */
    ...
    public Sdf.Contact Raycast(Vector3 from, Vector3 direction, float maxDistance, bool computeMaterials, int maxSteps = 128, float margin = 1e-2f, bool forceZeroBlendUnion = false)
    {
      ...
      Sdf.EvaluateRaycast(Sdf.AsyncMode.None, this, s_aSingleRaySync, s_aSingleContactSync, margin, m_aSdfBrush, m_numSdfBrushes, m_aSdfBrushMaterial, m_aabbTree.NodePods, m_aabbTree.Root, computeMaterials, maxSteps, SurfaceShift, forceZeroBlendUnion);
    ...
    {
      UpdateComputeData();
      var hJob = Sdf.EvaluateRaycast(Sdf.AsyncMode.Async, this, casts, results, margin, m_aSdfBrush, m_numSdfBrushes, m_aSdfBrushMaterial, m_aabbTree.NodePods, m_aabbTree.Root, computeMaterials, maxSteps, SurfaceShift, forceZeroBlendUnion);
      m_jobQueue.Add(hJob);
      return hJob;
    }

    /*
    public Sdf.EvalJobHandle RaycastAsyncInputCopied(NativeArray<Sdf.Ray> casts, NativeArray<Sdf.Contact> results, bool computeMaterials, int maxSteps = 128, float margin = 1e-2f, bool forceZeroBlendUnion = false)
    {
      UpdateComputeData();
      var hJob = Sdf.EvaluateRaycast(Sdf.AsyncMode.AsyncInputCopied, this, casts, results, margin, m_aSdfBrush, m_numSdfBrushes, m_aSdfBrushMaterial, m_aabbTree.NodePods, m_aabbTree.Root, computeMaterials, maxSteps, SurfaceShift, forceZeroBlendUnion);
      m_jobQueue.Add(hJob);
      return hJob;
    }
    */
    ...
    {
      UpdateComputeData();
      Sdf.EvaluateRaycastChain(Sdf.AsyncMode.None, this, chain, s_aSingleContactSync, margin, m_aSdfBrush, m_numSdfBrushes, m_aSdfBrushMaterial, m_aabbTree.NodePods, m_aabbTree.Root, computeMaterials, maxSteps, SurfaceShift);
      return s_aSingleContactSync[0];
    }
    ...
    {
      UpdateComputeData();
      var hJob = Sdf.EvaluateRaycastChain(Sdf.AsyncMode.Async, this, chain, result, margin, m_aSdfBrush, m_numSdfBrushes, m_aSdfBrushMaterial, m_aabbTree.NodePods, m_aabbTree.Root, computeMaterials, maxSteps, SurfaceShift);
      m_jobQueue.Add(hJob);
      return hJob;
    }

    /*
    public Sdf.EvalJobHandle RaycastChainAsyncInputCopied(NativeArray<Vector3> chain, NativeArray<Sdf.Contact> result, bool computeMaterials, int maxSteps = 128, float margin = 1e-2f)
    {
      UpdateComputeData();
      var hJob = Sdf.EvaluateRaycastChain(Sdf.AsyncMode.AsyncInputCopied, this, chain, result, margin, m_aSdfBrush, m_numSdfBrushes, m_aSdfBrushMaterial, m_aabbTree.NodePods, m_aabbTree.Root, computeMaterials, maxSteps, SurfaceShift);
      m_jobQueue.Add(hJob);
      return hJob;
    }
    */
    ...
    public Sdf.Contact SnapToSurface(Vector3 p, float maxSurfaceDistance, bool computeMaterials, int maxSteps = 128, float margin = 1e-2f)
    {
      ...
      samples[0] = p;
      var hJob = Sdf.EvaluateSnapToSurface(Sdf.AsyncMode.None, this, samples, results, margin, m_aSdfBrush, m_numSdfBrushes, m_aSdfBrushMaterial, m_aabbTree.NodePods, m_aabbTree.Root, computeMaterials, maxSurfaceDistance, maxSteps, SurfaceShift);
      hJob.Complete();
    ...
    {
      UpdateComputeData();
      var hJob = Sdf.EvaluateSnapToSurface(Sdf.AsyncMode.Async, this, samples, results, margin, m_aSdfBrush, m_numSdfBrushes, m_aSdfBrushMaterial, m_aabbTree.NodePods, m_aabbTree.Root, computeMaterials, maxSurfaceDistance, maxSteps, SurfaceShift);
      m_jobQueue.Add(hJob);
      return hJob;
    }

    /*
    public Sdf.EvalJobHandle SnapToSurfaceAsyncInputCopied(NativeArray<Vector3> samples, NativeArray<Sdf.Contact> results, float maxSurfaceDistance, bool computeMaterials, int maxSteps = 128, float margin = 1e-2f)
    {
      UpdateComputeData();
      var hJob = Sdf.EvaluateSnapToSurface(Sdf.AsyncMode.AsyncInputCopied, this, samples, results, margin, m_aSdfBrush, m_numSdfBrushes, m_aSdfBrushMaterial, m_aabbTree.NodePods, m_aabbTree.Root, computeMaterials, maxSurfaceDistance, maxSteps, SurfaceShift);
      m_jobQueue.Add(hJob);
      return hJob;
    }
    */
